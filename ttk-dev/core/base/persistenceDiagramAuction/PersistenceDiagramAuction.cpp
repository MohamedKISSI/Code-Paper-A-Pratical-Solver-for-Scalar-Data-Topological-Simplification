...
double ttk::PersistenceDiagramAuction::initLowerBoundCost(const int kdt_index) {
  // From first diagram to second one
  lowerBoundCost_ = 0;
  ...
  // From second diagram to first one
  /*double lowerBoundCost2 = 0;
  std::cout << "From second diagram to first one" << std::endl;
  ...
  if(use_kdt_) {
    ...
    kdt->build(coordinates.data(), bidders_.size(), dimension);
  }
  for(unsigned int i = 0; i < goods_.size(); ++i) {
    if(goods_[i].isDiagonal())
      continue;
    ...
    if(use_kdt_) {
      std::array<double, 5> coordinates;
      goods_[i].GetKDTCoordinates(geometricalFactor_, coordinates);
      kdt->getKClosest(1, coordinates, neighbours, costs, kdt_index);
      bestIndex = neighbours[0]->id_;
      bestCost = goods_[i].cost(bidders_[bestIndex], wasserstein_,
                                geometricalFactor_, nonMatchingWeight_);
    } else {
      for(unsigned int j = 0; j < bidders_.size(); ++j) {
        double const cost = goods_[i].cost(
          bidders_[j], wasserstein_, geometricalFactor_, nonMatchingWeight_);
        if(cost < bestCost)
          bestCost = cost;
      }
    }
    ...
    // Compare with diagonal good
    Good g{goods_[i].x_, goods_[i].y_, true, -goods_[i].id_ - 1};
    g.SetCriticalCoordinates(bidders_[i].GetCriticalCoordinates());
    g.projectOnDiagonal();
    double const cost
      = goods_[i].cost(g, wasserstein_, geometricalFactor_, nonMatchingWeight_);
    if(cost < bestCost) {
      bestCost = cost;
      bestIndex = -1;
    }

    if(bestIndex != -1 and goods_[i].x_ != bidders_[bestIndex].x_
       and goods_[i].y_ != bidders_[bestIndex].y_
       and bidders_[bestIndex].x_ != bidders_[bestIndex].y_) {
      std::cout << i << " : " << bestIndex << " : " << bestCost << std::endl;
      std::cout << goods_[i].x_ << ", " << goods_[i].y_ << " : "
                << bidders_[bestIndex].x_ << ", " << bidders_[bestIndex].y_
                << std::endl;
    }

    // Update lower bound
    lowerBoundCost2 += bestCost;
  }

  // Get highest lower bound
  lowerBoundCost_ = std::max(lowerBoundCost1, lowerBoundCost2);*/
